...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px 0 10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto","order":"1"}'>
	  ...
	  <el-button v-if="btnAuth('config','删除') && selectedIds.length > 0" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#f56c6c","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="danger" @click="batchDelete"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-delete"></i>批量删除</el-button>
    </el-form>
	<div class="select2" :style='{"width":"100%","padding":"20px 0 10px","background":"#fff","height":"auto","order":"1"}'>
	  <div :style='{"padding":"0","margin":"4px 0","borderColor":"#eee","background":"none","borderWidth":"0 0 1px","width":"100%","position":"relative","borderStyle":"solid","height":"auto"}' class="list">
	    <div :style='{"padding":"0 5px","lineHeight":"32px","fontSize":"14px","color":"#333","display":"inline-block"}' class="label">批量操作：</div>
	...
	<div class="list" :style='{"width":"70%","margin":"0 auto 20px","background":"#fff","flex":"1","order":"4"}'>
		...
		<div class="list2 index-pv1" :style='{"padding":"0 0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"padding":"25px","boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.3)","margin":"10px 0","background":"#fff","display":"flex","width":"calc(50% - 10px)","fontSize":"0","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" class="list-item animation-box">
				<el-checkbox v-model="item.checked" @change="handleItemCheck(item)"></el-checkbox>
				<div @click.stop="toDetail(item)" style="flex: 1;">
...
<script>
  export default {
	...
	handleItemCheck(item) {
		...
		// 检查是否全选
		this.selectAll = this.dataList.length > 0 && this.dataList.every(item => item.checked);
	},
	// 处理全选
	handleSelectAll(val) {
		this.dataList.forEach(item => {
	...
	batchDelete() {
		...
		this.$confirm('确定要删除选中的轮播图吗？', '提示', {
			...
			this.$http.post('config/deleteBatch', {ids: this.selectedIds}).then(res => {
				if (res.data.code == 0) {
					this.$message({
						message: '删除成功',
						type: 'success',
